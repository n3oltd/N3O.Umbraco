...
| Item | Location | Required? | Notes |
|---|---|---|---|
| `uSync/v9/` (175 files) | `src/DemoSite/DemoSite.Web/uSync/v9/` | **No (safe to remove)** | Superseded by `uSync/v17/`. **Three claims previously in this cell were wrong and are retracted:** there is **no** `v9/.ignore` marker anywhere under this tree; "uSync skips it" was never demonstrated; and a `RootFolder` override to `uSync/v17/` is a **no-op** — `uSyncSettings.RootFolder` already defaults to `$"uSync/v{Version.Major}/"`, so uSync 17 computes `uSync/v17/` on its own (the generated schema's `default` annotation is stale documentation, not behaviour). What remains true: `uSync/v9` is only the `LegacyFolder`, reachable through explicit legacy paths and not a normal import, and its 174 tracked files are v13 exports naming v14-removed editors (`MediaPickerLegacy`, `HTMLEditor`, `RichtextEditor`, six `Nested*`, a `Macros/` folder). Deleting it is dead-weight cleanup, **not** a live import risk. |
