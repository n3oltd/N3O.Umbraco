...
>
> **Correction (2026-06-16):** the per-project `.StaticAssets` branches were since re-cut onto the **`Apps/` layout** and the **`@n3o/backoffice-core`** runtime name (the earlier `ClientApp` / `@n3o/auth-fetch` wording above is obsolete). They remain **pre-_workspace-unification_**, however: no root workspace `package.json`, explicit per-app `dependencies`/`devDependencies`, and the shared Vite config imported via a relative path (`../../../build/vite-config`) rather than the `@n3o/build` package. So the full npm-workspace unification is still the only piece deferred to the solution-wide PR. **Update (2026-06-16):** `v17-N3O.Umbraco.ReactRuntime`'s entire `N3O.Umbraco.Cms` directory has since been **synced to `v17-Talha`** (so it now carries the Cms restructure — `Cms/Apps/N3O.Umbraco.BackofficeCore`, ReactRuntime under `Cms/Build/` — plus the audit + visibility changes), and the repo-root `Directory.Build.targets` was added to it. `#850` (Extensions) is merged, so it has no live branch.
