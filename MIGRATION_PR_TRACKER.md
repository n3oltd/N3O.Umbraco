...
|---|---|---|---|
| **ReactRuntime (auth + visibility)** — now the de-facto Cms+Extensions carrier post-#850 | `v17-N3O.Umbraco.ReactRuntime` | `cf1bc8ffb` | **Foundation — must merge first.** Shared `@n3o/auth-fetch` (`createAuthFetch` + `UmbAuthFetchMixin`) + `N3O.Condition.WorkspaceVisibility` condition. **2026-06-16:** its entire `N3O.Umbraco.Cms` was **synced to `v17-Talha`** (Cms restructure: `Apps/N3O.Umbraco.BackofficeCore`, ReactRuntime→`Build/`; + audit changes; + the typed-visibility condition & retired bespoke DynamicListView condition). Also brought: the **Extensions visibility files** (`WorkspaceVisibilityController` + `WorkspaceVisibilityRes` + `DynamicListViewApiController`) and the repo-root **`Directory.Build.targets`** (was missing). Other `N3O.Umbraco.Extensions` files remain ~11 behind (not visibility-related). |
