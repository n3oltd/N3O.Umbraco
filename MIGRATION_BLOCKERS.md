...
**Status (session 10, 2026-06-02):** ✅ MOVED TO STANDALONE CLI — the conversion **no longer runs on application startup**. `N3ONestedContentMigrationPlan` + `NestedContentToBlockListMigration` are now **commented out** (banner points to the tool); committed as `0d6c2aede`. The logic lives in an independent console app — a sibling folder of this repo: **`D:\AI Migration Test\N3O.Umbraco.NestedContentMigration.Cli`** (net8, standalone — only `Microsoft.Data.SqlClient` + `Newtonsoft.Json`, NO Umbraco/N3O reference; not in the .sln). This lets the data migration run **offline** against the DB and be **tested against a live Umbraco 13 site without upgrading**.

Key behaviours: `--format v13|v17` (v13 = udi-based Block List values + omits the `propertyEditorUiAlias` column that doesn't exist on v13; v17 = key-based, mirrors the old on-startup output); schema auto-detection refuses `--format v17` on a v13 DB; whole run is one transaction; `--dry-run` executes then rolls back; `--apply` commits; converts NC data types **in place** to Block List + rewrites `umbracoPropertyData.textValue`. **Recommended path:** on a live v13 site run `--format v13`, render/verify in the v13 backoffice, then upgrade Umbraco normally (its own migrations convert udi→key). See the tool's `README.md` + `CLAUDE.md`.

⚠️ **CLI spec-vs-code gap (follow-up):** the tool's README (intentionally edited by Talha) documents several behaviours **not yet implemented in the code** — `NC_MIGRATE_CONNECTION` env-var fallback; hard **all-or-nothing rollback if any value fails** (the code currently logs the failure, skips that value, and commits the rest); a generated-key count + "nested Nested Content" + "unparseable config" warnings in the summary; and truly idempotent "never overwrite a non-NC value" (the converter currently rewrites a non-array value to an empty Block List). Reconcile before using on real data.

⚠️ Still **UNVALIDATED**: the NC-JSON → Block List **value/config shape** against a real installed Umbraco 13.x/17.x `BlockListConfiguration`, and the udi→key upgrade conversion — the demo DB has 0 Nested Content data types so the transform never ran against real content. **Always dry-run then apply on a restored copy** and confirm both the backoffice editor and the front-end render before production. After `--apply`, rebuild NuCache + Examine (raw SQL doesn't refresh them).

**Earlier (session 4, now superseded by the CLI):** the migration was registered as a startup `PackageMigrationPlan`; smoke-testing fixed its SQL for the v17 schema (below).
...
| 06 | Nested Content DB migration | **Moved to standalone offline CLI** (session 10) — on-startup plan disabled; `N3O.Umbraco.NestedContentMigration.Cli` (sibling repo) with `--format v13\|v17`, dry-run/transaction. ⚠️ README spec-vs-code gap + value/config shape still need a real legacy-DB dry-run. | Critical |
