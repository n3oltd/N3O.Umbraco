...
export class OpayoClient {
    ...
    getApplePaySession(): Promise<ApplePaySessionRes> {
        let url_ = this.baseUrl + "/umbraco/api/Opayo/applePaySession";
        url_ = url_.replace(/[?&]$/, "");
        ...
        return this.http.fetch(url_, options_).then((_response: Response) => {
            return this.processGetApplePaySession(_response);
        });
    }
    ...
    protected processGetApplePaySession(response: Response): Promise<ApplePaySessionRes> {
        ...
        if (status === 200) {
            return response.text().then((_responseText) => {
            let result200: any = null;
            result200 = _responseText === "" ? null : JSON.parse(_responseText, this.jsonParseReviver) as ApplePaySessionRes;
            return result200;
            });
        } else if (status === 400) {
            return response.text().then((_responseText) => {
        ...
        return Promise.resolve<ApplePaySessionRes>(null as any);
    }
