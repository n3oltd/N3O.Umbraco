...
;
baseUrl: string;
jsonParseReviver: (function (key, value) { return any; }) | undefined;
undefined;
...
add(req, AddToCartReq);
Promise < void  > {
    let: let, url_: url_,
    url_: url_,
    const: content_ = JSON.stringify(req),
    let: let, options_: options_,
    return: this.http.fetch(url_, options_).then(function (_response) {
        return _this.processAdd(_response);
    })
};
processAdd(response, Response);
Promise < void  > {
    const: status = response.status,
    let: let,
    _headers: any = {},
    if: function (response) { },
    : .headers && response.headers.forEach
};
{
...
processGetSummary(response, Response);
Promise < CartSummaryRes > {
    const: status = response.status,
    let: let,
    _headers: any = {},
...
return Promise.resolve(null);
remove(allocationNumber, string);
Promise < void  > {
    let: let, url_: url_,
    if: function (allocationNumber) { }
} === undefined || allocationNumber === null;
...
processRemove(response, Response);
Promise < void  > {
    const: status = response.status,
    let: let,
    _headers: any = {},
