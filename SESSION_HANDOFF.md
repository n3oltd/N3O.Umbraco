...
*Updated: 2026-06-02 (session 10) — use this to orient the next session*
...
- **NC→BlockList migration moved to a standalone offline CLI** (commit `0d6c2aede`). The on-startup `PackageMigrationPlan` (`N3ONestedContentMigrationPlan` + `NestedContentToBlockListMigration` in `N3O.Umbraco.Extensions/Migrations/`) is **commented out** — it **no longer runs at startup**. The tool is an independent project **outside this repo** (sibling folder): **`D:\AI Migration Test\N3O.Umbraco.NestedContentMigration.Cli`** — net8, standalone (only `Microsoft.Data.SqlClient` + `Newtonsoft.Json`, NO Umbraco/N3O ref; not in the .sln; builds 0 errors; has its own `README.md` + `CLAUDE.md`). It does the same SQL in-place data-type conversion + `umbracoPropertyData.textValue` rewrite, plus: **`--format v13|v17`** (v13 = udi-based values, omits the v14+ `propertyEditorUiAlias` column → works on a live Umbraco 13 DB; v17 = key-based), **schema auto-detection** (refuses `--format v17` on a v13 DB), and a **single transaction with a `--dry-run` rollback**. Recommended path: convert a v13 site in place with `--format v13`, verify in the v13 backoffice, then upgrade Umbraco normally (its own migrations convert udi→key). This is the new home for the BLOCKER-06 work (the per-site data migration is now an explicit offline step, not auto-run).
  - ⚠️ **CLI README ≠ code (follow-up):** the tool's README (intentionally edited by Talha) now specifies behaviours the **code does not yet implement** — `NC_MIGRATE_CONNECTION` env-var fallback; **hard all-or-nothing rollback if any single value fails** (the code currently logs+skips the failed value and commits the rest — the README's safety promise is not met); a generated-key count + "nested Nested Content" + "unparseable config" warnings in the run summary; and truly idempotent "never overwrite a non-NC value" (the converter currently rewrites a non-array value to an empty Block List). Reconcile the code to the README before any real run.
  - ⚠️ Still **unvalidated** against real NC data / a real `BlockListConfiguration` and the udi→key upgrade — dry-run then apply on a **restored copy**, verify backoffice + front-end, and rebuild NuCache + Examine after `--apply`.
