...
namespace N3O.Umbraco.Extensions.Routing;

/// <summary>
///     Provides the default <see cref="IPublishedRouter" /> implementation.
/// </summary>
[Obsolete("Remove when https://github.com/umbraco/Umbraco-CMS/issues/12834#issue-1338670897 is resolved")]
public class OurPublishedRouter : IPublishedRouter {
    ...
    /// </summary>
    public OurPublishedRouter(IOptionsMonitor<WebRoutingSettings> webRoutingSettings,
    ...
    private DomainAndUri? FindDomain(Uri uri, out string? defaultCulture) {
        ...
        using (var context = _umbracoContextFactory.EnsureUmbracoContext()) {
            ...
            domains = domains?.Where(IsPublishedContentDomain)
                              .ToList();
    ...
    internal bool FindTemplateRenderingEngineInDirectory(DirectoryInfo? directory, string alias, string[] extensions) {
        ...
        return directory.GetFiles()
                        .Any(f => extensions.Any(e => f.Name.InvariantEquals(alias + e)));
    }
    ...
    internal async Task<bool> FindPublishedContent(IPublishedRequestBuilder request) {
        ...
        try {
            ...
            foreach (IContentFinder contentFinder in _contentFinders) {
                if (_logger.IsEnabled(LogLevel.Debug)) {
                    _logger.LogDebug("Finder {ContentFinderType}",
                                     contentFinder.GetType()
                                                  .FullName);
                }
            ...
            if (_logger.IsEnabled(LogLevel.Debug)) {
                _logger.LogDebug("Found? {Found}, Content: {PublishedContentId}, Template: {TemplateAlias}, Domain: {Domain}, Culture: {Culture}, StatusCode: {StatusCode}",
                                 found,
    ...
    private bool FollowInternalRedirects(IPublishedRequestBuilder request) {
        ...
        if (request.PublishedContent.HasProperty(global::Umbraco.Cms.Core.Constants.Conventions.Content
                                                        .InternalRedirectId) == false) {
            return false;
        }
        ...
        using (var context = _umbracoContextFactory.EnsureUmbracoContext()) {
                ...
                if (_logger.IsEnabled(LogLevel.Debug)) {
                    _logger.LogDebug("FollowInternalRedirects: Failed to redirect to id={InternalRedirectId}: value is not an int nor a GuidUdi.",
                                     request.PublishedContent
                                            .GetProperty(global::Umbraco.Cms.Core.Constants.Conventions.Content
                                                                .InternalRedirectId)
                                           ?.GetSourceValue());
                }
            }
            ...
            if (internalRedirectNode == null) {
                ...
                redirect = true;
                if (_logger.IsEnabled(LogLevel.Debug)) {
                    _logger.LogDebug("FollowInternalRedirects: Redirecting to id={InternalRedirectId}",
                                     internalRedirectId);
                }
            }
    ...
    private void FindTemplate(IPublishedRequestBuilder request, bool contentFoundByFinders) {
        ...
        var altTemplate = useAltTemplate
                              ? _requestAccessor.GetRequestValue(global::Umbraco.Cms.Core.Constants.Conventions.Url
                                                                        .AltTemplate)
                              : null;
        ...
        if (string.IsNullOrWhiteSpace(altTemplate)) {
            ...
            if (request.PublishedContent.IsAllowedTemplate(_fileService,
                                                           _contentTypeService,
                ...
                if (template != null) {
                    ...
                    if (_logger.IsEnabled(LogLevel.Debug)) {
                        _logger.LogDebug("FindTemplate: The alternative template with alias={AltTemplate} does not exist, ignoring.",
                                         altTemplate);
                    }
                }
            } else {
                _logger.LogWarning("FindTemplate: Alternative template {TemplateAlias} is not allowed on node {NodeId}, ignoring.",
                                   altTemplate,
    ...
    private ITemplate? GetTemplate(int? templateId) {
        ...
        if (_logger.IsEnabled(LogLevel.Debug)) {
            _logger.LogDebug("GetTemplateModel: Got template id={TemplateId} alias={TemplateAlias}",
                             template.Id,
                             template.Alias);
        }
    ...
    private void FollowExternalRedirect(IPublishedRequestBuilder request) {
        ...
        // don't try to find a redirect if the property doesn't exist
        if (request.PublishedContent.HasProperty(global::Umbraco.Cms.Core.Constants.Conventions.Content.Redirect)
            == false) {
            return;
        }

        var redirectId = request.PublishedContent.Value(_publishedValueFallback,
                                                        global::Umbraco.Cms.Core.Constants.Conventions.Content.Redirect,
                                                        defaultValue: -1);
