...
public class GoogleSearchSearcher : ISearcher {
    private const int PageSize = 10;

    private readonly IContentCache _contentCache;
    private readonly IAppPolicyCache _appCache;

    public GoogleSearchSearcher(IContentCache contentCache, IAppPolicyCache appCache) {
    ...
    private (long TotalResults, IEnumerable<SearchResult> Results) GetCachedResults(string query,
                                                                                    int start,
                                                                                    int number) {
        var cacheKey = CacheKey.Generate<GoogleSearchSearcher>(nameof(Search), query, start, number);
    ...
    private (long TotalResults, IEnumerable<SearchResult> Results) ExecuteQuery(string query,
        ...
        try {
            ...
            return (0, Enumerable.Empty<SearchResult>());
        }
    }
}
