...
namespace N3O.Umbraco.Giving.Donations.Content {
    public class FundDonationOption : UmbracoContent<FundDonationOption> {
        ...
        public IEnumerable<PriceHandle> SinglePriceHandles => GetCollectionAs(x => x.SinglePriceHandles);

        public bool HideRegular => GetValue(x => x.HideRegular);
        public IEnumerable<PriceHandle> RegularPriceHandles => GetCollectionAs(x => x.RegularPriceHandles);
