...
export class N3oDataImportElement extends UmbElementMixin(LitElement) {
    @state()
    private _show: string = 'form';
    ...
    @state()
    private _datePatterns: DatePattern[] = [];

    @state()
    private _datePattern: DatePattern | null = null;

    @state()
    private _moveUpdatedContentToCurrentLocation: boolean = false;

    @state()
    private _importableProperties: ImportableProperty[] = [];

    @state()
    private _errorMessages: string[] | null = null;
    ...
    async #init(): Promise<void> {
        ...
        const datePatterns = (await res.json()) as DatePattern[];

        this._datePatterns = datePatterns;
        this._datePattern = datePatterns[0] ?? null;
    }
    ...
    #onDatePatternChange(event: Event): void {
        const id = (event.target as HTMLSelectElement).value;
        this._datePattern = this._datePatterns.find((x) => x.id === id) ?? null;
    }

    #onMoveChange(event: Event): void {
        this._moveUpdatedContentToCurrentLocation = (event.target as HTMLInputElement).checked;
    }

    #onPropertyToggle(property: ImportableProperty, event: Event): void {
        property.selected = (event.target as HTMLInputElement).checked;
        this.requestUpdate();
    }
    ...
    async #import(): Promise<void> {
        ...
        // renderRoot is ShadowRoot at runtime; cast is safe for LitElement components.
        const root = this.renderRoot as ShadowRoot;
        const csvFile = root.getElementById('csvFile') as HTMLInputElement;
        const zipFile = root.getElementById('zipFile') as HTMLInputElement;
        ...
        if (result.status === 200) {
            ...
            this.#processingError((await result.json()) as string | string[]);
        }
    }

    async #getStorageToken(input: HTMLInputElement): Promise<unknown> {
    ...
    #processingError(messages: string | string[]): void {
                                              ...
                                              <li>
                                                  <label>
                                                      <input
                                                          ...
                                                          @change=${(e: Event) => this.#onPropertyToggle(property, e)} />
