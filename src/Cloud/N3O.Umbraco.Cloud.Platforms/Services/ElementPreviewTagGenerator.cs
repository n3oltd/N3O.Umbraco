...
public abstract class ElementPreviewTagGenerator : PreviewTagGenerator {
    ...
    protected override void PopulatePreviewData(IReadOnlyDictionary<string, object> content,
        ...
        var designationUdi = content[AliasHelper<ElementContent>.PropertyAlias(x => x.Designation)]?.ToString();
        ...
        publishedDonationForm.Dimension1 = GetDataListValue<FundDimension1Value>(content, AliasHelper<ElementContent>.PropertyAlias(x => x.Dimension1))?.Name;
        publishedDonationForm.Dimension2 = GetDataListValue<FundDimension2Value>(content, AliasHelper<ElementContent>.PropertyAlias(x => x.Dimension2))?.Name;
        publishedDonationForm.Dimension3 = GetDataListValue<FundDimension3Value>(content, AliasHelper<ElementContent>.PropertyAlias(x => x.Dimension3))?.Name;
        publishedDonationForm.Dimension4 = GetDataListValue<FundDimension4Value>(content, AliasHelper<ElementContent>.PropertyAlias(x => x.Dimension4))?.Name;
