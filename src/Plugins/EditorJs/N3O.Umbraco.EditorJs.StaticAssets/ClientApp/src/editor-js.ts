...
// All editorjs tools are imported directly and bundled by Vite.
...
// ---- media/link picker result shapes (unverified — see FLAGs at top) ----
...
export class N3oEditorJsElement extends UmbElementMixin(LitElement) {
    ...
    // eslint-disable-next-line @typescript-eslint/no-explicit-any
    #editor: any;
    #editorId = 'skrivlet-editor-' + randomUUID();
    // FLAG: UMB_MODAL_MANAGER_CONTEXT consumer callback type — using `any` then casting at call site.
    // eslint-disable-next-line @typescript-eslint/no-explicit-any
    #modalManager: any;
    ...
    #getInitialData(): object {
        ...
        if (!this.#value) {
            ...
            try {
                return JSON.parse(this.#value) as object;
            } catch (e) {
    ...
    #stopUmbracosInterferingHotKeys(): void {
        if (!this.shadowRoot) {
            return;
        }
        const editableElements = this.shadowRoot.querySelectorAll(
    ...
    async #openMediaPicker(tool: { applyMediaSelection(item: MediaPickerResultItem): void }): Promise<void> {
        if (!this.#modalManager) {
            return;
        }

        // FLAG: UMB_MEDIA_PICKER_MODAL open() + onSubmit() result shape is cast — not verified against Umbraco 17 types.
    ...
    async #openLinkPicker(tool: { wrap(range: Range, url: string): void }, range: Range): Promise<void> {
        if (!this.#modalManager) {
            return;
        }

        // FLAG: UMB_LINK_PICKER_MODAL open() + onSubmit() result shape is cast — not verified against Umbraco 17 types.
        const modal = this.#modalManager.open(this, UMB_LINK_PICKER_MODAL, {
            data: {
                config: {},
            },
        });
    ...
    #buildTools(): object {
        ...
        class UmbracoLinkTool {
            ...
            surround(range: Range): void {
                ...
                void host.#openLinkPicker(this, range);
            }
        ...
        class UmbracoImageTool {
            ...
            render(): HTMLDivElement {
                ...
                this.button.addEventListener('click', () => {
                    void host.#openMediaPicker(this);
                });
    ...
    override firstUpdated(): void {
        ...
        const holder = this.shadowRoot.getElementById(this.#editorId);
        if (!holder) {
            return;
        }

        // EditorJS ships a .d.ts but its `holder` type accepts string|HTMLElement;
        // passing HTMLElement directly is fine at runtime but tsc may complain — cast to any for the whole
        // config object to avoid fighting mismatched generic overloads in the EditorJS typings.
        ...
        this.#editor = new (EditorJS as any)({
            ...
            onChange: () => {
                this.#stopUmbracosInterferingHotKeys();
    ...
    #openFullscreen(): void {
        ...
        const container = this.shadowRoot.getElementById(this.#editorId);
    ...
    override render() {
        return html`
            <div class="skriv-let">
                <div id=${this.#editorId} class="skriv-let__container"></div>
                <button class="skriv-let__fullscreen-button" @click=${this.#openFullscreen} type="button">
                    <svg
