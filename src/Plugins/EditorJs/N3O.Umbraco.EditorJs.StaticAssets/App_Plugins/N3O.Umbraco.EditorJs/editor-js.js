...
// Side-effect import: exposes EditorJS and all the bundled tools on window
// (EditorJS, Header, RawTool, Checklist, List, Embed, Quote, CodeTool, DragDrop, Paragraph, AlignmentTool).
import './editorjs-all.js';
...
class N3oEditorJsElement extends UmbElementMixin(LitElement) {
    static properties = {
        value: { type: String },
    };
    ...
    async #openLinkPicker(tool, range) {
        if (!this.#modalManager) {
            return;
        }
    ...
    #buildTools() {
        ...
        class UmbracoLinkTool {
            ...
            surround(range) {
                ...
                host.#openLinkPicker(this, range);
            }
        ...
        class UmbracoImageTool {
            ...
            applyMediaSelection(item) {
                ...
                this.data.width = parseInt(item.width); // sometimes a string for some reason?
                this.data.height = parseInt(item.height);
        ...
        class EmbedWithUI extends window.Embed {
            static get toolbox() {
                return {
                    title: 'Video',
            ...
            render() {
                if (!this.data.service) {
                    ...
                    input.addEventListener('paste', (event) => {
                        ...
                        const service = Object.keys(window.Embed.services).find((key) =>
                            window.Embed.services[key].regex.test(url)
                        );
                        if (service) {
...
customElements.define(elementName, N3oEditorJsElement);

export default N3oEditorJsElement;
export { N3oEditorJsElement };
