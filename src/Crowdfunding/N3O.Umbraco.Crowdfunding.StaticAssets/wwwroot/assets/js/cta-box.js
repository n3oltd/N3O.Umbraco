function n3o_cdf_handleDonationForm() {
  ...
  ctaFunds.forEach((ctaFund) => {
    ...
    ctaInput.addEventListener('click', function (event) {
      const _input = ctaAmount.querySelector("input[type='number']");

      if (_input.readOnly) {
        ctaCheckbox.checked = true;
        const changeEvent = new Event('change', { bubbles:true });
        ctaCheckbox.dispatchEvent(changeEvent);
      }
    });
