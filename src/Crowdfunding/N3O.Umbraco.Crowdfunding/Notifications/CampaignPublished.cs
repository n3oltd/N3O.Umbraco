...
public class CampaignPublished : INotificationAsyncHandler<ContentPublishedNotification> {
    public Task HandleAsync(ContentPublishedNotification notification, CancellationToken cancellationToken) {
        ...
        // command to execute a SQL UPDATE CampaignName WHERE CampaignId = '' query on the online
        // and offline contributions tables. We can actually get away with executing this update
        // in all cases as the volume will be low, we don't really care if it ends up being a no op.
