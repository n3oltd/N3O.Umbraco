...
public class CrowdfundingPageController : ApiController {
    ...
    [HttpPut("{pageId:guid}/{propertyAlias}/boolean")]
    [ProducesResponseType(StatusCodes.Status404NotFound)]
    public async Task UpdateBooleanField([FromRoute] Guid pageId, [FromRoute] string propertyAlias, PropertyValue value) {
        await _fundraisingPages.UpdatePropertyAsync(pageId,
                                                    null,
                                                    x => x.Content.Boolean(propertyAlias).Set((bool) value.Value));
    }

    [HttpPut("{pageId:guid}/{propertyAlias}/dateTime")]
    [ProducesResponseType(StatusCodes.Status404NotFound)]
    public async Task UpdateDateTimeField([FromRoute] Guid pageId, [FromRoute] string propertyAlias, PropertyValue value) {
        await _fundraisingPages.UpdatePropertyAsync(pageId,
                                                    null,
                                                    x => x.Content.DateTime(propertyAlias).SetDateTime((DateTime) value.Value));
    }

    [HttpPut("{pageId:guid}/{propertyAlias}/dropdown")]
    [ProducesResponseType(StatusCodes.Status404NotFound)]
    public async Task UpdateDropdownField([FromRoute] Guid pageId, [FromRoute] string propertyAlias, PropertyValue value) {
        await _fundraisingPages.UpdatePropertyAsync(pageId,
                                                    null,
                                                    x => x.Content.Dropdown(propertyAlias).Set(value.Value.ToString()));
    }

    [HttpPut("{pageId:guid}/{propertyAlias}/email")]
    [ProducesResponseType(StatusCodes.Status404NotFound)]
    public async Task UpdateEmailField([FromRoute] Guid pageId, [FromRoute] string propertyAlias, PropertyValue value) {
        await _fundraisingPages.UpdatePropertyAsync(pageId,
                                                    null,
                                                    x => x.Content.Email(propertyAlias).Set(value.Value.ToString()));
    }

    [HttpPut("{pageId:guid}/{propertyAlias}/label")]
    [ProducesResponseType(StatusCodes.Status404NotFound)]
    public async Task UpdateLabelField([FromRoute] Guid pageId, [FromRoute] string propertyAlias, PropertyValue value) {
        await _fundraisingPages.UpdatePropertyAsync(pageId,
                                                    null,
                                                    x => x.Content.Label(propertyAlias).Set((string) value.Value));
    }

    [HttpPut("{pageId:guid}/{propertyAlias}/multipleText")]
    [ProducesResponseType(StatusCodes.Status404NotFound)]
    public async Task UpdateMultipleTextField([FromRoute] Guid pageId, [FromRoute] string propertyAlias, PropertyValue value) {
        await _fundraisingPages.UpdatePropertyAsync(pageId,
                                                    null,
                                                    x => x.Content.MultipleText(propertyAlias).Set((IEnumerable<string>) value.Value));
    }
