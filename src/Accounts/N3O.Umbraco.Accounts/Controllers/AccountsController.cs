...
namespace N3O.Umbraco.Giving.Accounts.Controllers {
    ...
    public class AccountsController : LookupsController<AccountsLookupsRes> {
        ...
        [HttpGet("consentOptions")]
        public async Task<ActionResult<IEnumerable<ConsentOption>>> GetConsentOptions() {
            var res = await _mediator.SendAsync<GetConsentOptionsQuery, None, IEnumerable<ConsentOption>>(None.Empty);
