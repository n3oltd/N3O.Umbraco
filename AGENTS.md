...
- **npm workspace** (`src/package.json`) orchestrated by **Turborepo** (`turbo run build`), each package built by **Vite** to `dist/`, then MSBuild copies `dist/` → `wwwroot/App_Plugins/` (see `Directory.Build.targets`). Built JS is gitignored.
- `frontend/build-config` (`@repo/build-config`) — shared Vite/TS config; extend tsconfig **by name** (`@repo/build-config/tsconfig[-react]`).
- `frontend/backoffice-core` (`@n3oltd/backoffice-core`) — shared Lit/TS (auth-fetch + the workspace-visibility condition). **Externalized** and shipped as its own App_Plugins bundle (resolved at runtime via the import map).
- `frontend/backoffice-ui` (`@n3oltd/backoffice-ui`) — **shared React wrappers over UUI** (see below). **Bundled** into each consuming app (not externalized).
- `N3O.Umbraco.ReactRuntime` — ships React 19 as a single shared ESM bundle + import map (`react`, `react-dom` are **externalized** and resolved to this one instance; they are `peerDependencies` in every React plugin).
- Exemplar plugin: `Data/N3O.Umbraco.Data/frontend/{data-export,data-import,...}` — the template every other plugin follows.
...
- Frontend only: `cd src && npm ci && npx turbo run build --env-mode=loose` (8 packages; each app runs `tsc --noEmit` then Vite).
...
- PR #863 (`v17-N3O.Umbraco.ReactRuntime`) — the React-client foundation + Data exemplar.
- PR #875 — the UUI-wrapper refactor of the Data apps (stacked on #863).
